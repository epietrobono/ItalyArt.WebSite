...
const Api = {
  ...
  GetMapMonuments: async (category, search, NELat, NELon, SWLat, SWLon) => {    
    const postBody={Category: category, Search:search, NELat:NELat, NELon:NELon, SWLat:SWLat, SWLon:SWLon};
    const response = await instance.post(baseUrl+"api/MapMonuments", postBody);
    return response.data.results;
  },
  GetHeader: async () => {
